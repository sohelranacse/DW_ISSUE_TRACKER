...
class EUsers_List {
    ...
    static $c_user_id = false;

    static function show($where = '', $order = '', $from_add = '', $template = '', $from_group = '', $redirectIfSingle=false, $c_user_id=false)
    {
        global $g;
        global $g_user;
        global $p;
        ...
        if (Wall::isProfileWall($display)
            ...
            elseif ($display == "profile" || $display == "profile_info") { // do

                $listTmpl = array(
                    'main' => $g['tmpl']['dir_tmpl_main'] . '_edit_profile.html'
                );
                if (Common::isParseModule('profile_head')){
                    $listTmpl['profile_head'] = $g['tmpl']['dir_tmpl_main'] . '_profile_head.html';
                }
                if (Common::isParseModule('profile_photos_init')){ // do
